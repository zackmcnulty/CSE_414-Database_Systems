...
//    SparkSession spark = SparkSession.builder().appName("HW6").getOrCreate();
 ...
 //         .javaRDD().saveAsTextFile(output);
  ...
  public static Dataset<Row> Q1 (SparkSession spark, String dataFile) {
    ...
    Dataset<Row> r = spark.sql("SELECT DISTINCT F.destcityname FROM flights F WHERE f.origincityname LIKE '%Seattle%'");
    //r.show();
    return r;
  }
  ...
  public static JavaRDD<Row> Q2 (SparkSession spark, String dataFile) {
    ...
    // your code here
    JavaRDD<Row> seattle_flights = d.filter(t -> t.getString(ORIGIN_CITY_NAME).contains("Seattle"));

    JavaRDD<Row> dest_cities = seattle_flights.map(t -> RowFactory.create(t.getString(DEST_CITY_NAME)));

    return dest_cities.distinct();
  }
  ...
  public static JavaPairRDD<Tuple2<String, Integer>, Integer> Q3 (SparkSession spark, String dataFile) {
    ...
    // 2) create pairs <Key, value> = <(city, monthID), 1> for each flight
    // 3) reduce pairs by summing those with same keys.
    JavaRDD<Row> noncanceled = d.filter(t -> t.getInt(CANCELLED) != 1);// only consider noncancelled flights or flights with NULL cancelled entry

    JavaPairRDD<Tuple2<String, Integer>, Integer> pairs =  // create pairs ((city, MonthID), 1)
    		noncanceled.mapToPair(t -> new Tuple2<Tuple2<String,Integer>, Integer>(
					new Tuple2<String,Integer>(t.getString(ORIGIN_CITY_NAME), t.getInt(MONTH)), 1));

    return pairs.reduceByKey((sum, next) -> (sum + next)); //(for each pair with same key = (city, month) add the values (all the ones))
  }
  ...
  public static Tuple2<String, Integer> Q4 (SparkSession spark, String dataFile) {
    ...
    // 1) mapToPair to create pairs <city, dest>
    // 2) remove duplicates so all <city, dest> pairs are unique
    // 3) map to pair <city, 1> for each pair ^
    // 4) reduce by key by summing the 1's ^
    // 5) find max ^; find associated max city
    // 6) filter d for only flights from that city ^
    // 7) return cityname, d.count
    //

    JavaPairRDD<String, String> connected_cities = 
	    	d.mapToPair(t -> new Tuple2(t.getString(ORIGIN_CITY_NAME), t.getString(DEST_CITY_NAME)));
    connected_cities = connected_cities.distinct(); // for each city, has distinct pairs of origin -> dest city

    JavaPairRDD<String, Integer> city_counts = 
	    connected_cities.mapToPair(t -> new Tuple2(t._1, 1));

    JavaPairRDD<String, Integer> total_counts = city_counts.reduceByKey((sum, next) -> (sum + next)); //counts of # cities each city flies to directly (city, count)


    // make comparator; defined at bottom of this program
    CityComparator city_comparer = new CityComparator(); 
    String max_city = total_counts.max(city_comparer)._1;

    // filter for flights that have best_city as their origin city 
    d = d.filter(t -> t.getString(ORIGIN_CITY_NAME).equals(max_city));
    return new Tuple2<String, Integer>(max_city, (int) d.count()) ;//new Tuple2<String, Integer>(best_city, (Integer) d.count()); // return flight count + city name
  }

  public static JavaPairRDD<String, Double> Q5 (SparkSession spark, String dataFile) {

    JavaRDD<Row> d = spark.read().parquet(dataFile).javaRDD();
    // 1) filter out all null delay times
    // 2) create pairs <city, <delay time, 1>>
    // 3) reduce by key (city) to <city, <total delay time, flight count>>
    // 4) map to <city, ave delay time>
    
    JavaRDD<Row> non_null = d.filter(t -> !t.isNullAt(DEP_DELAY)); // filter out flights with null delay time

    JavaPairRDD<String, Tuple2<Integer, Integer>> city_pairs = 
    		non_null.mapToPair(t -> new Tuple2<String, Tuple2<Integer, Integer>> (t.getString(ORIGIN_CITY_NAME), new Tuple2(t.getInt(DEP_DELAY), 1)));

    JavaPairRDD<String, Tuple2<Integer, Integer>> total_delays = city_pairs.reduceByKey( (val1, val2) -> new Tuple2(val1._1 + val2._1, val1._2 + val2._2 ));

    return total_delays.mapToPair(t -> new Tuple2(t._1, ((double) t._2._1) / t._2._2));
  }
 ...
 |-- div5tailnum: integer (nullable = true)
   */
	...
	public static class CityComparator implements Serializable, Comparator<Tuple2<String, Integer>> {
